...
function maskEmail(e) {
  try {
    ...
    const dom = dParts[0];
    const tld = dParts.slice(1).join('.') || 'com';
    const dMask = dom[0] + '*'.repeat(Math.max(1,dom.length-1));
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const mapped = (data||[]).map(r=>({
      score:r.score, total:r.total, ms:r.ms, at:r.created_at, email: maskEmail(r.user_email)
    }));
